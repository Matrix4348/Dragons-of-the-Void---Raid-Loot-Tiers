...
function create_detailed_div(raid_name,mode,raid_difficulty){
    ...
    if(raid_list[raid_name][mode]["Loot format"]=="EHL"){
        ...
        if ( raid_list[raid_name][mode]["Has extra drops"]["On-hit drops"] || raid_list[raid_name][mode]["Has extra drops"]["Loot expansion"] ){
            ...
            var x3 = raid_list[raid_name][mode]["Has extra drops"]["On-hit drops"] ? "Yes" : "No", x4 = raid_list[raid_name][mode]["Has extra drops"]["Loot expansion"] ? "Yes" : "No";
...
function sanitized_object(o){
    ...
    if( typeof(o) == "string" ){ sane_object = sanitized_string(o); }
        ...
        if( Array.isArray(o) ){ sane_object = []; }
        else if( o == null ){ sane_object = null; }
        else{ sane_object = {}; }
        for(let k in o){
            sane_object[sanitized_object(k)] = sanitized_object(o[k]);
        }
    }
    else{ sane_object = o; }
    return sane_object;
