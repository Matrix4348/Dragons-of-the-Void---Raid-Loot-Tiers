...
// Workaround to an unexpected compatibility issue with some GM_* functions in Greasemonkey.
...
function create_in_raid_div(raid_name,raid_difficulty){
    ...
    if(raid_list[raid_name].loot_format=="EHL"){
        ...
        t.innerHTML=`<td>`+raid_list[raid_name].raid_type+`</td> <td>Latest loot table known by the script (be careful in case of a rerun): `+raid_list[raid_name]["Loot table"]+`</td>`;
    }
    td.appendChild(t);
                            ...
                            if(typeof(document.getElementsByClassName("dotv-btn dotv-btn-xl active")[0])==='object'){
                                if(document.getElementsByClassName("dotv-btn dotv-btn-xl active").length>1){
                                    ...
                                    if(raid_name in raid_list){
                                        if(raid_list[raid_name].loot_format=="EHL"||raid_list[raid_name].raid_type=="World raid"){
                                            in_raid_button();
