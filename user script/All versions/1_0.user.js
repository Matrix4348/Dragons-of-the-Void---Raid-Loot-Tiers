...
var raid_list={
    ...
    "Naga Karamati":new raid("Large","","EHL","2,000 | 10,000 | 20,000 | 80,000 | 120,000 | 175,000 | <b>240,000FS</b> | 320,000 | 450,000 | 580,000 | 840,000 | 1,350,000 | 1,920,000","15,000 | 50,000 | 120,000 | 180,000 | 240,000 | 400,000 | 600,000 | <b>720,000=FS</b> | 1,120,000 | 1,600,000 | 2,400,000 | 3,200,000 | 4,400,000 | 5,760,000","??? | <b>2,160,000=FS</b> | ??? | 17,280,000?",""),
    ...
    for(var k in raid_list){
        if( ((size=="All")||(raid_list[k].raid_size==size))&&((type=="All")||(raid_list[k].raid_type==type)) ){
            if(raid_list[k].loot_format=="EHL"){
                ...
                if(diffsum==3){
                    tle=t.insertRow();
                    tle.innerHTML=`<td rowspan="`+diffsum+`">`+k+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_type+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_size+`</td> <td>Easy</td> <td>`+raid_list[k].Easy+`</td>`;
                    tlh=t.insertRow();
                    tlh.innerHTML=`<td>Hard</td> <td>`+raid_list[k].Hard+`</td>`;
                    tll=t.insertRow();
                    tll.innerHTML=`<td>Legendary</td> <td>`+raid_list[k].Legendary+`</td>`;
                }
                else if(difficulties_to_display.Easy==1){
                    tle=t.insertRow();
                    tle.innerHTML=`<td rowspan="`+diffsum+`">`+k+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_type+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_size+`</td> <td>Easy</td> <td>`+raid_list[k].Easy+`</td>`;
                    if(difficulties_to_display.Hard==1){ tlh=t.insertRow(); tlh.innerHTML=`<td>Hard</td> <td>`+raid_list[k].Hard+`</td>`; }
                    if(difficulties_to_display.Legendary==1){ tll=t.insertRow(); tll.innerHTML=`<td>Legendary</td> <td>`+raid_list[k].Legendary+`</td>`; }
                }
                else if(difficulties_to_display.Hard==1){
                    tlh=t.insertRow();
                    tlh.innerHTML=`<td rowspan="`+diffsum+`">`+k+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_type+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_size+`<td>Hard</td> <td>`+raid_list[k].Hard+`</td>`;
                    tll=t.insertRow();
                    tll.innerHTML=`<td>Legendary</td> <td>`+raid_list[k].Legendary+`</td>`;
                }
                else if(difficulties_to_display.Legendary==1){
                    tll=t.insertRow();
                    tll.innerHTML=`<td rowspan="`+diffsum+`">`+k+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_type+`</td> <td rowspan="`+diffsum+`">`+raid_list[k].raid_size+`<td>Legendary</td> <td>`+raid_list[k].Legendary+`</td>`;
                }
                else{}
            }
...
function createAboutTab(){
    ...
    https://docs.google.com/spreadsheets/d/1wSilYulbp3M3f2eHHhPmDlM5AiCHFQPbNvYJU2jWYZE/edit#gid=581592972</a> (a spreadsheet I am not related to), so if there are holes there, then there
    will likely be holes here, too.</p>
    <br>
    <p>If I forgot anything or if you find any bugs, do not forget to report them to me, either on the script page on Greasyfork (if you have an account)
    or anywhere else you might find me. I am also always open to suggestions.</p>`;
}
